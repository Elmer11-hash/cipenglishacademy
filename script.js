...
// Add event listeners to all the links with the 'promo-link2' class
document.querySelectorAll(".promo-link2").forEach(function (link) {
  link.addEventListener("click", function (e) {
    e.preventDefault();
    const imageUrl = link.href; // Get the Imgur URL
    const fileName = link.getAttribute("data-image"); // Get the file name from the data attribute

    const downloadLink = document.createElement("a");
    downloadLink.href = imageUrl; // Set the Imgur image URL
    downloadLink.download = fileName + ".jpg"; // Set the file name for the download
